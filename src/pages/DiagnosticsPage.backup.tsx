    ...
    <div className="pb-24 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">
            {report?.phase || 'Phase 1'} Diagnostics
          </h1>
          <p className="text-dex-text-secondary">
            {report?.phase === 'Phase 2'
              ? 'Real-time data and wallet integration verification'
              : 'Real-time data implementation verification'
            }
          </p>
        </div>
        <Button
      ...
      {report && (
        <>
          ...
          {/* Phase 2 Metrics */}
          {report.phase2Metrics && (
            <Card className="bg-dex-dark/80 border-dex-primary/30">
              <CardHeader>
                <CardTitle className="text-white">Phase 2 Wallet Integration</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Wallet Connectivity</div>
                    <div className="flex items-center gap-2">
                      {report.phase2Metrics.walletConnectivityEnabled ? (
                        <CheckCircle className="w-4 h-4 text-green-500" />
                      ) : (
                        <XCircle className="w-4 h-4 text-red-500" />
                      )}
                      <span className="text-white font-semibold">
                        {report.phase2Metrics.walletConnectivityEnabled ? 'Enabled' : 'Disabled'}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Connected Wallets</div>
                    <div className="flex items-center gap-2">
                      {getStatusIcon(report.phase2Metrics.connectedWalletsCount, 1)}
                      <span className="text-white font-semibold">{report.phase2Metrics.connectedWalletsCount}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Real Transactions</div>
                    <div className="flex items-center gap-2">
                      {report.phase2Metrics.realTransactionsEnabled ? (
                        <CheckCircle className="w-4 h-4 text-green-500" />
                      ) : (
                        <XCircle className="w-4 h-4 text-red-500" />
                      )}
                      <span className="text-white font-semibold">
                        {report.phase2Metrics.realTransactionsEnabled ? 'Enabled' : 'Disabled'}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Supported Networks</div>
                    <div className="text-white font-semibold">
                      {report.phase2Metrics.supportedNetworks.length}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Wallet Service Status</div>
                    <div className="text-white font-semibold">{report.phase2Metrics.walletServiceStatus}</div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-sm text-dex-text-secondary">Transaction Service Status</div>
                    <div className="text-white font-semibold">{report.phase2Metrics.transactionServiceStatus}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="bg-dex-dark/80 border-dex-primary/30">
            <CardHeader>
              <CardTitle className="text-white">{report.phase} Summary</CardTitle>
            </CardHeader>
            <CardContent>
